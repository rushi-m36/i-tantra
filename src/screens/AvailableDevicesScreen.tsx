...
export default function AvailableDevicesScreen() {
  ...
  const handleSelfCall = async () => {
    setLoading(true);
    try { await selfCall(); } catch (error) { Alert.alert("Self Test Failed", String(error)); } finally { setLoading(false); }
  };
  ...
  const handleAddDevice = () => {
    ...
    const newDevice: Device = { id: deviceIp.trim(), name: deviceName.trim(), ip: deviceIp.trim(), port: 5555, status: "available", lastSeen: Date.now() };
    void handleCall(newDevice);
    setDeviceIp(""); setDeviceName(""); setShowAddDevice(false);
  };

  if (callState !== "idle" && callState !== "incoming") {
    return <View className="flex-1 bg-white px-6 pt-16"><Text className="text-2xl font-bold text-gray-900">{callState === "calling" ? "Calling..." : "Connecting..."}</Text>{currentDevice && <Text className="mt-4 text-lg text-gray-700">{currentDevice.name}</Text>}</View>;
  }
  ...
  return (
    <View className="flex-1 bg-white px-6 pt-16">
      ...
      <Modal visible={showAddDevice} transparent animationType="slide">
        <View className="flex-1 bg-black/50 p-6 pt-32"><View className="rounded-lg bg-white p-6">
          ...
          <View className="mt-6 flex-row justify-end gap-3">
            <TouchableOpacity onPress={() => { setShowAddDevice(false); setDeviceIp(""); setDeviceName(""); }} className="rounded-lg bg-gray-300 px-4 py-2"><Text className="font-semibold text-gray-900">Cancel</Text></TouchableOpacity>
            <TouchableOpacity onPress={handleAddDevice} className="rounded-lg bg-blue-500 px-4 py-2"><Text className="font-semibold text-white">Add & Call</Text></TouchableOpacity>
          </View>
        </View></View>
      </Modal>
    </View>
