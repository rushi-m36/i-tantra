  ...
  return <View style={{ flex: 1, backgroundColor: BG }}>
    ...
    <ScrollView ref={scrollViewRef} style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 16, paddingTop: 22, paddingBottom: 24 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" onContentSizeChange={() => scrollViewRef.current?.scrollToEnd({ animated: true })}>
      {messages.length === 0 ? <View style={{ paddingHorizontal: 8, paddingTop: 30 }}><Text style={{ fontSize: 15, color: MUTED }}>No messages yet.</Text><Text style={{ marginTop: 5, fontSize: 13, color: "#777" }}>Hold the microphone below to speak.</Text></View> : messages.map((item: ChatMessageType, index) => {
        ...
        return <View key={safeItem.id} style={{ marginBottom: 14, width: "100%" }} onLayout={(e) => { const { width, height } = e.nativeEvent.layout; console.log("[RENDER DEBUG] row", { id: safeItem.id, text: safeItem.text, textLength: safeItem.text.length, width: Math.round(width * 100) / 100, height: Math.round(height * 100) / 100 }); }}>
          <View style={{ width: "100%", alignItems: own ? "flex-end" : "flex-start" }} onLayout={(e) => { const { width, height } = e.nativeEvent.layout; console.log("[RENDER DEBUG] alignParent", { id: safeItem.id, width: Math.round(width * 100) / 100, height: Math.round(height * 100) / 100, alignItems: own ? "flex-end" : "flex-start" }); }}>
            ...
            <View onLayout={(e) => { const { width, height } = e.nativeEvent.layout; console.log("[RENDER DEBUG] bubble", { id: safeItem.id, text: safeItem.text, textLength: safeItem.text.length, width: Math.round(width * 100) / 100, height: Math.round(height * 100) / 100, maxWidth: Math.round(SCREEN_WIDTH * 0.82 * 100) / 100 }); }} style={{ alignSelf: own ? "flex-end" : "flex-start", maxWidth: SCREEN_WIDTH * 0.82, backgroundColor: own ? BUBBLE : "#1d1d1d", borderRadius: 22, paddingHorizontal: 16, paddingVertical: 11, borderWidth: 1, borderColor: BORDER }}>
              <View style={{ borderWidth: 1, borderColor: "#777", backgroundColor: "transparent" }} onLayout={(e) => { const { width, height } = e.nativeEvent.layout; console.log("[RENDER DEBUG] textContainer", { id: safeItem.id, width: Math.round(width * 100) / 100, height: Math.round(height * 100) / 100 }); }}>
                <Text style={{ fontSize: 16, lineHeight: 23, color: FG }} onLayout={(e) => { const { width, height } = e.nativeEvent.layout; console.log("[RENDER DEBUG] text", { id: safeItem.id, text: safeItem.text, textLength: safeItem.text.length, width: Math.round(width * 100) / 100, height: Math.round(height * 100) / 100 }); }} onTextLayout={(e) => { const lines = e.nativeEvent.lines; console.log("[RENDER DEBUG] textLayout", { id: safeItem.id, text: safeItem.text, textLength: safeItem.text.length, lineCount: lines.length, lineWidths: lines.map((line) => Math.round(line.width * 100) / 100), lineTexts: lines.map((line) => line.text) }); }}>{safeItem.text}</Text>
              </View>
            </View>
          </View>
        </View>;
