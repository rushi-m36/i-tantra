...
const MESSAGE_MAX_WIDTH = SCREEN_WIDTH * 0.85;
...
export default function CommunicationScreen() {
  ...
  return <View style={{ flex: 1, backgroundColor: BG }}>
    <View style={{ paddingHorizontal: 22, paddingTop: 48, paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: BORDER }}><View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}><View style={{ flex: 1, minWidth: 0 }}><Text style={{ fontSize: 11, fontWeight: "700", fontFamily: ANDROID_FONT, letterSpacing: 2, color: MUTED }}>CONNECTED</Text><Text style={{ marginTop: 5, fontSize: 21, fontWeight: "700", fontFamily: ANDROID_FONT, color: FG, flexShrink: 1 }}>{currentDevice?.name || "Device"}</Text></View><TouchableOpacity onPress={endCall} activeOpacity={0.8} style={{ borderWidth: 1, borderColor: FG, borderRadius: 18, paddingHorizontal: 15, paddingVertical: 8, flexShrink: 0 }}><Text style={{ fontWeight: "600", fontFamily: ANDROID_FONT, color: FG }}>End</Text></TouchableOpacity></View></View>
    ...
    <Modal visible={showTextInput} transparent animationType="fade" onRequestClose={() => setShowTextInput(false)}><View style={{ flex: 1, alignItems: "center", justifyContent: "center", backgroundColor: "rgba(0,0,0,0.85)", padding: 20 }}><View style={{ width: "100%", maxWidth: 420, backgroundColor: BG, borderWidth: 1, borderColor: BORDER, borderRadius: 24, padding: 22 }}><Text style={{ fontSize: 20, fontWeight: "700", fontFamily: ANDROID_FONT, color: FG }}>Message</Text><TextInput autoFocus placeholder="Type your message" value={messageText} onChangeText={setMessageText} multiline placeholderTextColor="#777" style={{ marginTop: 16, minHeight: 100, width: "100%", borderWidth: 1, borderColor: BORDER, borderRadius: 18, padding: 14, color: FG, fontFamily: ANDROID_FONT, fontWeight: "400", textAlignVertical: "top" }} /><View style={{ marginTop: 16, flexDirection: "row", gap: 10 }}><TouchableOpacity onPress={() => setShowTextInput(false)} activeOpacity={0.8} style={{ flex: 1, borderWidth: 1, borderColor: FG, borderRadius: 18, paddingVertical: 12 }}><Text style={{ textAlign: "center", fontWeight: "600", fontFamily: ANDROID_FONT, color: FG }}>Cancel</Text></TouchableOpacity><TouchableOpacity onPress={handleSendMessage} disabled={!messageText.trim()} activeOpacity={0.8} style={{ flex: 1, borderRadius: 18, backgroundColor: messageText.trim() ? FG : "#333", paddingVertical: 12 }}><Text style={{ textAlign: "center", fontWeight: "700", fontFamily: ANDROID_FONT, color: BG }}>Send</Text></TouchableOpacity></View></View></View></Modal>
  </View>;
}
