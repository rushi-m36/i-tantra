...
export class TCPService {
      ...
      try {
        server = TcpSocket.createServer((socket: any) => {
          ...
          socket.on("close", () => {
            ...
            if (!this.clientSocket) { this.stopHeartbeat(); this.connectionCallbacks.forEach((cb) => cb(false)); }
          });
        });
        this.server = server;
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        const socket = TcpSocket.createConnection(options, () => {
          if (settled) return;
          settled = true; this.clientSocket = socket;
          if (typeof socket.setTimeout === "function") socket.setTimeout(0);
          this.startHeartbeat(); console.log("Connected to device at", ip, port); this.connectionCallbacks.forEach((cb) => cb(true)); resolve();
        });
        this.clientSocket = socket;
        socket.on("data", (data: any) => {
          try { const dataStr = decodeTcpData(data); const { messages } = this.messageProtocol.processData(dataStr); messages.forEach((msg) => this.messageCallbacks.forEach((cb) => cb(msg))); }
          catch (error) { console.error("Failed to process TCP data:", error); }
        });
        socket.on("error", (err: any) => {
          console.error("TCP client socket error:", err);
          if (!settled) { settled = true; if (this.clientSocket === socket) this.clientSocket = null; reject(err); }
        });
        socket.on("close", () => {
          console.log("TCP client disconnected");
        ...
        if (!settled) { settled = true; this.clientSocket = null; reject(error); }
      }
    });
  }
  ...
  async cleanup(): Promise<void> {
    this.stopHeartbeat(); await this.disconnect(); await this.stopServer();
