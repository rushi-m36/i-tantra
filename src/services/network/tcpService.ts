...
export class TCPService {
    ...
    const startPromise = new Promise<number>((resolve, reject) => {
      ...
      const server = TcpSocket.createServer((socket: any) => {
        ...
        socket.on("data", (data: any) => {
          ...
          } catch (error) { console.error("TCP server data processing failed", error); }
        });
        socket.on("error", () => {});
        socket.on("close", () => {
      ...
      try {
        ...
        socket.on("data", (data: any) => {
          try {
            ...
            const { messages } = this.messageProtocol.processData(dataStr);
            messages.forEach((msg) => this.messageCallbacks.forEach((cb) => cb(msg)));
          } catch (error) { console.error("TCP client data processing failed", error); }
        });
  ...
  async stopServer(): Promise<void> {
    ...
    this.stopPromise = new Promise<void>((resolve) => {
      let finished = false;
      const finish = () => { if (finished) return; finished = true; if (this.server === server) this.server = null; resolve(); };
