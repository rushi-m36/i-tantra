...
import { getDeviceName, getIpAddress, getUniqueId } from "react-native-device-info";
...
const SCAN_HOST_START = 1;
const SCAN_HOST_END = 254;
...
export type DiscoveryPhase = "idle" | "nsd" | "tcp" | "nsd-found";
...
export class DeviceDiscovery {
  ...
  async getNetworkInfo(): Promise<{ ip: string; subnet: string; isWifi: boolean; fallback: boolean } | null> {
    try {
      ...
      try {
        if (LocalNetwork?.getLocalIPv4Addresses) nativeIps = (await LocalNetwork.getLocalIPv4Addresses()).filter((value) => this.isValidIpv4(value));
      } catch (error) { console.error("Native local IPv4 lookup failed:", error); }

      let deviceInfoIp: string | null = null;
      try { const value = await getIpAddress(); if (this.isValidIpv4(value)) deviceInfoIp = value; } catch {}
      ...
      if (privateIps.length === 0) return { ip: "192.168.43.1", subnet: "255.255.255.0", isWifi: true, fallback: true };
  ...
  setDiscoveryPhase(phase: DiscoveryPhase): void {
    this.scanStatus = { ...this.scanStatus, phase, scanning: phase === "tcp", currentIp: phase === "tcp" ? this.scanStatus.currentIp : null };
    this.notifyScanStatus();
  }
  ...
  startDiscovery(): void {
    ...
    this.cleanupInterval = setInterval(() => {
      ...
      this.discoveredDevices.forEach((device, id) => { if (now - device.lastSeen > DEVICE_TIMEOUT) { this.discoveredDevices.delete(id); changed = true; } });
  ...
  private startDiscoveryServer(): void {
    try {
      ...
      this.discoveryServer.listen({ port: DISCOVERY_PORT, host: "0.0.0.0", reuseAddress: true }, () => {});
  ...
  private async scanLocalNetwork(): Promise<void> {
    ...
    try {
      ...
      this.scanStatus = { scanning: true, currentIp: null, scanned: 0, total: addresses.length, found: this.discoveredDevices.size, phase: "tcp" };
      this.notifyScanStatus();
      ...
      for (let index = 0; index < addresses.length; index += MAX_CONCURRENT_SCANS) {
        ...
        this.scanStatus = { ...this.scanStatus, scanned, currentIp: addresses[scanned - 1] || null, found: this.discoveredDevices.size };
        this.notifyScanStatus();
      }
    } catch (error) { console.error("Local network scan failed:", error); }
    finally {
      this.scanInProgress = false;
      if (this.scanStatus.phase === "tcp") this.scanStatus = { ...this.scanStatus, scanning: false, currentIp: null, found: this.discoveredDevices.size };
      this.notifyScanStatus();
    }
  }

  private getCommonHotspotAddresses(): string[] {
    const prefixes = ["192.168.43", "192.168.137", "192.168.42"]; const addresses: string[] = [];
    for (const prefix of prefixes) for (let host = SCAN_HOST_START; host <= SCAN_HOST_END; host++) addresses.push(`${prefix}.${host}`);
    return addresses;
  }
  ...
  private getSubnetAddresses(ip: string, subnet: string): string[] {
    ...
    const prefix = ipParts.slice(0, 3).join("."); const addresses: string[] = [];
    for (let host = SCAN_HOST_START; host <= SCAN_HOST_END; host++) { const candidate = `${prefix}.${host}`; if (candidate !== ip) addresses.push(candidate); }
    return addresses;
  }

  private probeDevice(ip: string): Promise<void> {
    return this.probePort(ip, DISCOVERY_PORT, true).then((found) => found ? undefined : this.probePort(ip, TCP_PORT, false).then(() => undefined));
  }
  ...
  private probePort(ip: string, port: number, discoveryPort: boolean): Promise<boolean> {
    return new Promise((resolve) => {
      ...
      const finish = (found = false) => { if (settled) return; settled = true; if (timer) clearTimeout(timer); if (socket && !socket.destroyed) socket.destroy(); resolve(found); };
      try {
        ...
        socket.on("data", (data: any) => {
          try {
            ...
            const line = responseBuffer.split("\n")[0].trim(); if (!line) return;
            const response = JSON.parse(line) as DiscoveryResponse; const expectedMagic = discoveryPort ? DISCOVERY_REQUEST : TCP_PROBE;
        ...
        socket.on("error", () => finish(false)); socket.on("close", () => { if (!settled) finish(false); });
      } catch { finish(false); }
    });
  }
  ...
  stopDiscovery(): void {
    ...
    this.scanStatus = { ...this.scanStatus, scanning: false, currentIp: null, phase: "idle" };
    this.notifyScanStatus();
  }
