...
/** Native speech recognition with Android offline-model selection. */
  ...
  private async resolveAndroidLocale(requestedLocale: string): Promise<string> {
    ...
    try {
      ...
      if (installed.length === 0) {
        throw new Error(
          "No Android offline speech model is installed. Install an offline speech language in Android System Intelligence / On-device speech recognition settings.",
        );
      }
      ...
      const sameLanguage = installed.find(
        (locale) => locale.toLowerCase().split("-")[0] === requestedLanguage,
      );
      if (sameLanguage) return sameLanguage;

      const english = installed.find(
        (locale) => locale.toLowerCase() === DEFAULT_LOCALE.toLowerCase(),
      );
      if (english) {
        console.log(`Offline STT ${requestedLocale} is not installed; using ${english}`);
        return english;
      }

      console.log(`Offline STT ${requestedLocale} is not installed; using ${installed[0]}`);
      return installed[0];
    } catch (error) {
      if (error instanceof Error && error.message.startsWith("No Android offline")) {
        throw error;
      }
      console.warn("Could not query Android offline STT locales:", error);
      throw new Error(
        "Android offline speech recognition models could not be queried. Check that Android System Intelligence / On-device speech recognition is enabled.",
      );
    }
  }
  ...
  async listenForSpeech(locale = DEFAULT_LOCALE): Promise<string> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        console.log(`Starting STT: ${recognitionLocale} (offline)`);
