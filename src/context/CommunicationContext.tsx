...
export function CommunicationProvider({ children }: { children: React.ReactNode }) {
  const [devices, setDevices] = useState<Device[]>([]); const [scanStatus, setScanStatus] = useState<ScanStatus>({ scanning: false, currentIp: null, scanned: 0, total: 0, found: 0, phase: "idle" }); const [callState, setCallState] = useState<CallState>("idle"); const [messages, setMessages] = useState<ChatMessage[]>([]); const [currentDevice, setCurrentDevice] = useState<Device | null>(null); const currentDeviceRef = useRef<Device | null>(null); const [isConnected, setIsConnected] = useState(false); const [incomingCallFrom, setIncomingCallFrom] = useState<Device | null>(null); const [tcp, setTcp] = useState<TCPService | null>(null); const [discovery, setDiscovery] = useState<DeviceDiscovery | null>(null); const [deviceId, setDeviceId] = useState(""); const [deviceName, setDeviceName] = useState(""); const [refreshVersion, setRefreshVersion] = useState(0); const autoConnecting = useRef(false);
  ...
  const handleIncomingMessage = useCallback(async (msg: Message, disc: DeviceDiscovery) => {
    switch (msg.type) {
      ...
      case "call_accept": {
        ...
        const peer = currentDeviceRef.current;
        if (peer?.ip) {
          let reconnected = false;
          for (let attempt = 1; attempt <= 5 && !reconnected; attempt += 1) {
            try {
              await new Promise((resolve) => setTimeout(resolve, 300));
              await tcp?.connectToDevice(peer.ip, peer.port || TCP_PORT);
              reconnected = true;
              console.log("[CALL HANDOFF] Caller reconnected after remote accepted", peer.ip);
            } catch (error) {
              console.log(`[CALL HANDOFF] Caller reconnect attempt ${attempt} failed`, error);
            }
          }
          if (!reconnected) console.error("[CALL HANDOFF] Caller could not reconnect after remote accepted");
        }
        break;
      }
  ...
  const cancelCall = useCallback(async () => { if (!tcp) return; try { if (tcp.isConnected()) await tcp.sendMessage({ type: "call_end", senderId: deviceId, timestamp: Date.now() }); } catch (error) { console.error("Failed to cancel call:", error); } await tcp.disconnect(); setCallState("idle"); setCurrentDevice(null); currentDeviceRef.current = null; setIncomingCallFrom(null); }, [tcp, deviceId]);
  ...
  const rejectCall = useCallback(async () => { if (!tcp || !incomingCallFrom) return; try { await tcp.sendMessage({ type: "call_reject", senderId: deviceId, timestamp: Date.now() }); } catch (error) { console.error("Failed to reject call:", error); } await tcp.disconnect(); setCallState("idle"); setIncomingCallFrom(null); setCurrentDevice(null); currentDeviceRef.current = null; }, [tcp, incomingCallFrom, deviceId]);
